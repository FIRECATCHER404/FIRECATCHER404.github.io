const MAGIC = [0x42, 0x4c, 0x45, 0x4e];
const VERSION = 1;
const KDF_SCRYPT = 1;
...
function parsePackage(packageBytes) {
  ...
  for (let i = 0; i < MAGIC.length; i += 1) {
    if (packageBytes[i] !== MAGIC[i]) {
      throw new Error("Encrypted input is not a Baselen BLEN package.");
    }
  }
  ...
  if (version !== VERSION) throw new Error(`Unsupported Baselen version: ${version}.`);
